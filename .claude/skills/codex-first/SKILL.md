---
name: codex-first
description: "MANUAL ONLY — never auto-invoke. Load ONLY when the user explicitly runs /codex-first or asks to delegate to Codex. Routes implementation work to Codex CLI; Claude specs, reviews, verifies."
---

# Codex First

**Invocation: manual only.** Never auto-invoke this skill on task-matching. It applies ONLY when the user explicitly runs `/codex-first` or asks in words to delegate to Codex. Absent an explicit request, do the work in Claude as normal — never shell out to `codex` on your own.
